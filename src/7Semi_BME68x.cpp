...
BME68x_7Semi::BME68x_7Semi() {}

/**
 * Initialize BME68x sensor using I2C
 *
 ...
 */
bool BME68x_7Semi::begin(uint8_t address, TwoWire &i2cPort, uint32_t i2cSpeed, uint8_t sda, uint8_t scl) {
  ...
  BME68x_dev.intf_ptr = &i2c_h;
  BME68x_dev.read = i2cRead;
  BME68x_dev.write = i2cWrite;
  BME68x_dev.delay_us = delayUs;

  /**
   * Initialize Bosch BME68x driver
   *
   * Loads calibration data from sensor.
   */
  status = bme68x_init(&BME68x_dev);
  if (status != BME68X_OK)
    return false;
  ...
  status = bme68x_set_conf(&conf, &BME68x_dev);
  if (status != BME68X_OK)
    return false;
  ...
  bme68x_set_heatr_conf(BME68X_FORCED_MODE, &heatr, &BME68x_dev);
...
/**
 * Initialize BME68x sensor using SPI
 *
 ...
 */
bool BME68x_7Semi::beginSPI(uint8_t csPin, SPIClass &spiPort, uint32_t spiFreq,
  ...
  BME68x_dev.intf_ptr = &spi_h;
  BME68x_dev.read = spiRead;
  BME68x_dev.write = spiWrite;
  BME68x_dev.delay_us = delayUs;
 ...
 */
bool BME68x_7Semi::readChipID(uint8_t &id) {
 ...
 * - false : reset failed
 */
bool BME68x_7Semi::softReset()
{
  status = bme68x_soft_reset(&BME68x_dev);

  return (status == BME68X_OK);
}
 ...
 */
bool BME68x_7Semi::setOperationMode(uint8_t op_mode)
{
  status = bme68x_set_op_mode(op_mode, &BME68x_dev);
 ...
 * - false : read failed
 */
bool BME68x_7Semi::getOperationMode(uint8_t &op_mode)
{
  status = bme68x_get_op_mode(&op_mode, &BME68x_dev);
 ...
 *
 * Notes
 * - BME68x gas sensor requires heater for gas measurement
 ...
 */
bool BME68x_7Semi::setHeater(bool enable, uint16_t temp, uint16_t duration) {
  ...
  status = bme68x_set_heatr_conf(BME68X_FORCED_MODE, &heatr, &BME68x_dev);
 ...
 */
bool BME68x_7Semi::getHeater(bool &enable, uint16_t &temp, uint16_t &duration) {

  /** Read heater configuration from sensor */
  status = bme68x_get_heatr_conf(&heatr, &BME68x_dev);
 ...
 */
bool BME68x_7Semi::setHeaterProfile(uint8_t profile, uint16_t temp, uint16_t duration, bool enable) {
  ...
  status = bme68x_set_heatr_conf(BME68X_FORCED_MODE, &heaterProfiles[profile], &BME68x_dev);
 ...
 */
bool BME68x_7Semi::getHeaterProfile(uint8_t profile, uint16_t &temp, uint16_t &duration, bool &enable) {
 ...
 */
int8_t BME68x_7Semi::setHeater(const struct bme68x_heatr_conf *conf) {
  return bme68x_set_heatr_conf(BME68X_FORCED_MODE, conf, &BME68x_dev);
}
 ...
 * - Bosch driver status code
 */
int8_t BME68x_7Semi::getHeater(struct bme68x_heatr_conf *conf) {
  return bme68x_get_heatr_conf(conf, &BME68x_dev);
}
 ...
 */
bool BME68x_7Semi::setSampling(BME68x_sampling temp,
                               BME68x_sampling hum,
                               BME68x_sampling pres,
 ...
 * - false : read failed
 */
bool BME68x_7Semi::getSampling(BME68x_sampling &temp,
                               BME68x_sampling &hum,
                               BME68x_sampling &pres,
                               BME68x_filter &filterSetting)
{
  /** Read configuration from sensor */
  status = bme68x_get_conf(&conf, &BME68x_dev);

  if (status != BME68X_OK)
    return false;

  /** Copy configuration values */
  temp          = static_cast<BME68x_sampling>(conf.os_temp);
  hum           = static_cast<BME68x_sampling>(conf.os_hum);
  pres          = static_cast<BME68x_sampling>(conf.os_pres);
  filterSetting = static_cast<BME68x_filter>(conf.filter);
 ...
 * - false : configuration failed
 */
bool BME68x_7Semi::setODR(BME68x_odr odr)
{
 ...
 */
bool BME68x_7Semi::getODR(BME68x_odr &odr) const
{
  /** Return stored ODR configuration */
  odr = static_cast<BME68x_odr>(conf.odr);
 ...
 */
bool BME68x_7Semi::getMeasurementDuration(uint8_t op_mode, uint32_t &duration) {
  duration = bme68x_get_meas_dur(op_mode, &conf, &BME68x_dev);
  return true;
}
 ...
 * - false : configuration failed
 */
bool BME68x_7Semi::setForcedMode() {
  status = bme68x_set_op_mode(BME68X_FORCED_MODE, &BME68x_dev);
  return status == BME68X_OK;
}
 ...
 */
bool BME68x_7Semi::getData(bme68x_data &outData) {
  ...
  /** Calculate measurement duration */
  getMeasurementDuration(BME68X_FORCED_MODE, measure_duration);

  /** Wait for measurement completion */
  delay(measure_duration / 1000 + 1);

  /** Read sensor data */
  status = bme68x_get_data(BME68X_FORCED_MODE, &BME68x_Data, &data_size, &BME68x_dev);
 ...
 */
bool BME68x_7Semi::getData() {
  return getData(BME68x_Data);
}
 ...
 * - false : measurement failed
 */
bool BME68x_7Semi::getData(float &t, float &h, float &p, float &g) {

  if (!getData())
    return false;

  t = BME68x_Data.temperature;
  h = BME68x_Data.humidity;
  p = BME68x_Data.pressure / 100.0;
  g = BME68x_Data.gas_resistance;
 ...
 */
float BME68x_7Semi::getTemperature() {
 ...
 * - NAN if measurement failed
 */
float BME68x_7Semi::getPressure() {

  if (!getData())
    return NAN;

  return BME68x_Data.pressure / 100.0;
}
 ...
 * - NAN if measurement failed
 */
float BME68x_7Semi::getHumidity() {
 ...
 * - NAN if measurement failed
 */
float BME68x_7Semi::getGasResistance() {
 ...
 */
bool BME68x_7Semi::readForced(bme68x_data &outData)
{
  /** Set sensor to forced mode */
  if (bme68x_set_op_mode(BME68X_FORCED_MODE, &BME68x_dev) != BME68X_OK)
    return false;

  /** Calculate measurement duration */
  uint32_t measure_duration = bme68x_get_meas_dur(BME68X_FORCED_MODE, &conf, &BME68x_dev);
  ...
  if (bme68x_get_data(BME68X_FORCED_MODE, &BME68x_Data, &n_fields_local, &BME68x_dev) != BME68X_OK)
    return false;
 ...
 */
bool BME68x_7Semi::readParallel(bme68x_data *dataArray, uint8_t &fields)
{
  ...
  status = bme68x_set_op_mode(BME68X_PARALLEL_MODE, &BME68x_dev);

  if (status != BME68X_OK)
    return false;

  /** Calculate measurement duration */
  uint32_t meas_dur = bme68x_get_meas_dur(BME68X_PARALLEL_MODE, &conf, &BME68x_dev);
  ...
  status = bme68x_get_data(BME68X_PARALLEL_MODE, dataArray, &fields, &BME68x_dev);

  if (status != BME68X_OK || fields == 0)
    return false;
 ...
 */
int8_t BME68x_7Semi::i2cRead(uint8_t reg, uint8_t *data, uint32_t len, void *intf_ptr) {
 ...
 * - transmission result
 */
int8_t BME68x_7Semi::i2cWrite(uint8_t reg, const uint8_t *data, uint32_t len, void *intf_ptr) {
 ...
 * - 0 : read successful
 */
int8_t BME68x_7Semi::spiRead(uint8_t reg, uint8_t *data, uint32_t len, void *intf_ptr)
{
  /** Retrieve SPI handle */
 ...
 * - 0 : write successful
 */
int8_t BME68x_7Semi::spiWrite(uint8_t reg, const uint8_t *data, uint32_t len, void *intf_ptr) {
 ...
 */
void BME68x_7Semi::delayUs(uint32_t period, void *) {
